...
void TPSCameraAttachment::ProcessInput(Input *input, float delta) {
    ...
    if(angle_around_change != 0) {
        m_camera->setYaw(180 - static_cast<float>(m_entity->getTransform().GetRot().y +  m_camera->angle_around_attachment ));
    }

    float theta = m_entity->getTransform().GetRot().y + m_camera->angle_around_attachment;
    float offsetX = horizontalDistance * sin(glm::radians(theta));
    float offsetZ = horizontalDistance * cos(glm::radians(theta));
    m_camera->setXPos(m_entity->getTransform().GetPos().x - offsetX);
    m_camera->setZPos(m_entity->getTransform().GetPos().z - offsetZ);
    m_camera->setYPos(m_entity->getTransform().GetPos().y + verticalDistance + 10.0f);
