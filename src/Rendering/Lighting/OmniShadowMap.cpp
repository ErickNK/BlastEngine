//
// Created by erick on 9/6/18.
//

#include "OmniShadowMap.h"

OmniShadowMap::OmniShadowMap() :ShadowMap() {};

OmniShadowMap::~OmniShadowMap() = default;

bool OmniShadowMap::Init(GLuint width, GLuint height) {
    m_shadowHeight = height;
    m_shadowWidth = width;

    glGenFramebuffers(1, &m_frameBufferObject); //Create the shadow-map framebuffer

    glGenTextures(1, &m_shadowMapTexture); //Create shadow-map texture
    glBindTexture(GL_TEXTURE_CUBE_MAP, m_shadowMapTexture);

        //WRAP filter
        glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
        glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
        glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_R, GL_CLAMP_TO_EDGE);
        float shadowBorderColors[]{1.0f,1.0f,1.0f,1.0f}; //No shadows beyond what the shadow map does not cover
        glTexParameterfv(GL_TEXTURE_CUBE_MAP,GL_TEXTURE_BORDER_COLOR,shadowBorderColors);

        //Overlap filter
        glTexParameterf(
                GL_TEXTURE_CUBE_MAP,
                GL_TEXTURE_MIN_FILTER, // Applied when texture is further away/smaller
                GL_LINEAR_MIPMAP_LINEAR
        );
        glTexParameterf(
                GL_TEXTURE_CUBE_MAP,
                GL_TEXTURE_MAG_FILTER, // Applied when texture is closer/bigger
                GL_LINEAR
        );

        for(size_t i = 0; i < 6; i++){
            //Data
            glTexImage2D(
                    GL_TEXTURE_CUBE_MAP_POSITIVE_X + i,
                    0,
                    GL_DEPTH_COMPONENT,
                    m_shadowWidth, m_shadowHeight,
                    0,
                    GL_DEPTH_COMPONENT,
                    GL_FLOAT,
                    nullptr
            );
        }
        glBindFramebuffer(GL_FRAMEBUFFER,m_frameBufferObject); //Bind buffer for both drawing and reading

            glFramebufferTexture( //Tells buffer to draw its contents to a out shadow-map texture.
                    GL_FRAMEBUFFER,
                    GL_DEPTH_ATTACHMENT, // Tells buffer to write only depth data.
                    m_shadowMapTexture,
                    0
            );
            // No reading or drawing color
            glDrawBuffer(GL_NONE);
            glReadBuffer(GL_NONE);

            GLenum status = glCheckFramebufferStatus(GL_FRAMEBUFFER);
            if (status != GL_FRAMEBUFFER_COMPLETE) {
                printf("Framebuffer Error: &i\n", status);
                return false;
            }

        glBindFramebuffer(GL_FRAMEBUFFER, 0);
    glBindTexture(GL_TEXTURE_CUBE_MAP, 0);

    return true;
}

void OmniShadowMap::BindFrameBuffer()
{
    glBindFramebuffer(GL_FRAMEBUFFER, m_frameBufferObject); //Bind shadow buffer.
}

void OmniShadowMap::UnBindFrameBuffer()
{
    glBindFramebuffer(GL_FRAMEBUFFER, 0); //unBind shadow buffer.
}

void OmniShadowMap::UseShadowMap(GLenum textureUnit)
{
    // Activate the texture unit the texture is to load into
    glActiveTexture(GL_TEXTURE0 + textureUnit);

    // Bind the texture for use
    glBindTexture(GL_TEXTURE_CUBE_MAP, m_shadowMapTexture);
}
